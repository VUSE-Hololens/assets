...
{
    // inspector vars
    public string remoteIP = "10.66.134.150";
    public string remotePrimPort = "8888";
    public string remoteSecPort = "8889";
    public Receiver.Trans_Mode Mode;
    ...
    void Awake()
    {
        GameObject recObj = new GameObject();
        recObj.AddComponent<Receiver>();
        rec = recObj.GetComponent<Receiver>();

        rec.RemoteIP = remoteIP;
        rec.RemotePrimPort = remotePrimPort;
        rec.RemoteSecPort = remoteSecPort;
        rec.Mode = Mode;

        rec.Go();
        
        // old method - not a gameobject
