...
public class EFPDriver : MonoBehaviour {
    ...
    public Receiver SensorDataUpdater { get; private set; }
    public ReceiverDriver SensorDataUpdaterContatiner { get; private set; }
    ...
    private void Start()
    {
        SensorDataUpdaterContatiner = DataUpdateContainer.GetComponent<ReceiverDriver>();
        SensorDataUpdater = SensorDataUpdaterContatiner.rec;
        SensorDataHash = SensorDataUpdater.CurrentHash;
    }
    ...
    private void UpdateSensor()
    {
        ...
        if (SensorDataHash != SensorDataUpdater.CurrentHash || SensorDataHash == Receiver.INVALID_DATA_HASH)
        {
            SensorDataHash = SensorDataUpdater.CurrentHash;
            if (SensorDataHash != Receiver.INVALID_DATA_HASH)
            {
                Vector2Int pixels = SensorDataUpdater.Pixels;
                SensorDataWidth = pixels.x;
                SensorDataHeight = pixels.y;
                SensorData = SensorDataUpdater.Content;
            }
            else
            {
                SensorDataWidth = SensorDataHeight = SensorDataUpdaterContatiner.dummySize;
                SensorData = SensorDataUpdaterContatiner.dummyData;
            }

            // debug
            UnityEngine.Debug.Log(string.Format("EFPDriver pulling updated sensor data from Receiver. Hash: {0}", SensorDataHash));
        }
    }
