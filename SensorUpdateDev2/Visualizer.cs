...
public class Visualizer
{
    ...
    /// NOTE: copied from ProximitySensor/Intersector.cs. CachedVertex changed to straight-up Vector3.
    /// </summary>
    ...
    public static List<Content> CreateMarkers(List<PointValue<byte>> pointValues, float radius, 
        byte minValue, byte maxValue, Color minColor, Color maxColor)
    {
        List<Content> result = new List<Content>();

        foreach (PointValue<byte> pv in pointValues)
        {
